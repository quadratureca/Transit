...
public class GoTransit : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        try
        {
            ...
            _logger.LogInformation("Route.GoTransit running at: {time}", DateTimeOffset.Now);

            while (!stoppingToken.IsCancellationRequested)
            {
                try
                {
                    string goTransit = "https://assets.metrolinx.com/raw/upload/Documents/Metrolinx/Open%20Data/GO-GTFS.zip";
                            ...
                            using (StreamReader sr = new StreamReader(unzippedEntryStream))
                            {
                                ...
                                while (sr.Peek() != -1)
                                {
                                    ...
                                    if (line != null)
                                    {
                                        ...
                                        route.route_desc = string.Empty;
                                        route.route_type = fields[4];
                                        route.route_color = fields[5];
                                        route.route_text_color = fields[6];
