...
public class Hamilton : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        try
        {
            ...
            _logger.LogInformation("Route.Hamilton running at: {time}", DateTimeOffset.Now);

            while (!stoppingToken.IsCancellationRequested)
            {
                try
                {
                    string hamilton = "https://googlehsrdocs.hamilton.ca/";
                            ...
                            using (StreamReader sr = new StreamReader(unzippedEntryStream))
                            {
                                ...
                                while (sr.Peek() != -1)
                                {
                                    ...
                                    if (line != null)
                                    {
                                        ...
                                        route.route_id = fields[5];
                                        route.agency_id = fields[4];
                                        route.route_short_name = fields[8];
                                        route.route_long_name = fields[0];
                                        route.route_desc = fields[7];
                                        route.route_type = fields[1];
                                        route.route_url = fields[6];
                                        route.route_color = fields[3];
                                        route.route_text_color = fields[2];
