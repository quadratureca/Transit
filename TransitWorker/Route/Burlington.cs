...
public class Burlington : BackgroundService
{
    private readonly ILogger<Burlington> _logger;

    public Burlington(ILogger<Burlington> logger)
    {
        _logger = logger;
    }
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        try
        {
            ...
            _logger.LogInformation("Route.Burlington running at: {time}", DateTimeOffset.Now);

            while (!stoppingToken.IsCancellationRequested)
            {
                try
                {
                    string burlington = "https://opendata.burlington.ca/gtfs-rt/GTFS_Data.zip";

                    try
                    {
                        var x = client.GetStreamAsync(burlington);
                        data = x.Result;
                            ...
                            using (StreamReader sr = new StreamReader(unzippedEntryStream))
                            {
                                ...
                                while (sr.Peek() != -1)
                                {
                                    ...
                                    if (line != null)
                                    {
                                        ...
                                        route.agency_id = "Burlington";
                                        route.route_short_name = fields[1];
                                        route.route_long_name = fields[2].Replace("\"", "");
                                        route.route_desc = fields[3];
                                        route.route_type = fields[4];
                                        route.route_url = fields[5];
                                        route.route_color = fields[6];
                                        route.route_text_color = fields[7];
