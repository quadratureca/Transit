...
public class Brampton : BackgroundService
{
    private readonly ILogger<Brampton> _logger;

    public Brampton(ILogger<Brampton> logger)
    {
        _logger = logger;
    }
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        try
        {
            ...
            while (!stoppingToken.IsCancellationRequested)
            {
                try
                {
                    string brampton = "https://www.brampton.ca/EN/City-Hall/OpenGov/Open-Data-Catalogue/Documents/Google_Transit.zip";
                            ...
                            using (StreamReader sr = new StreamReader(unzippedEntryStream))
                            {
                                ...
                                while (sr.Peek() != -1)
                                {
                                    ...
                                    if (line != null)
                                    {
                                        ...
                                        route.route_id = fields[0].Split("-")[0];
                                        route.agency_id = "Brampton Transit";
